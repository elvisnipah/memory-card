    ...
    function addToScore(event, cardID) {
        ...
        for (let i = 0; i < gameData.length; i++) {
            if (gameData[i].id === cardID && gameData[i].clicked) {
                ...
                gameData[i].id === cardID &&
                gameData[i].clicked === false
            ) {
        ...
        <main>
            <div className="App">
                <div className="heading">
                    Get points by clicking on an image but don't click on it any
                    more than once!
                </div>
                {currentScore === 8 ? (
                    <p className="congrats-text">
                        Congrats, you win! Click another card to start again
                    </p>
                ) : (
                    ""
                )}
